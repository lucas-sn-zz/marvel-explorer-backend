...
class ComicModel(db.Model):
    ...
    def find_user_comics(cls,id_user,name=None):
            if name == None:
                ...
                result = cls.query.filter_by(id_user=id_user).filter(cls.name.like(f"{name}%")).all()
            return [value.id_comic for value in result]

    @classmethod
    def find_user_comics_by_ids(cls, id_user,ids_comic_list):
        result = cls.query.filter(cls.id_comic.in_(ids_comic_list)).filter_by(id_user=id_user).all()
        return [value.id_comic for value in result]
