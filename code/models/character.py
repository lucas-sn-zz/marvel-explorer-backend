...
class CharacterModel(db.Model):
    ...
    def find_user_characters(cls,id_user,name=None):
        if name == None:
            ...
            cls.query.filter_by(id_user=id_user).filter(cls.query.name.like(f"{name}%")).all()
        return [value.id_character for value in result]
        

    @classmethod
    def find_user_characters_by_ids(cls, id_user,ids_character_list):
        result = cls.query.filter(cls.id_character.in_(ids_character_list)).filter_by(id_user=id_user).all()
        return [value.id_character for value in result]
